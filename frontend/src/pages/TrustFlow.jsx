...
function FlowBar({ label, value, color, desc }) {
  return (
    <div style={{ background: '#FFFFFF', border: '1px solid #DAE0EA', borderRadius: 12, padding: '18px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 8 }}>
        <div style={{ fontWeight: 700, fontSize: 14, color: '#0F172A' }}>{label}</div>
        <div style={{ fontSize: 26, fontWeight: 800, color }}>{value}</div>
      </div>
      <div style={{ height: 8, background: '#E5E9F2', borderRadius: 4, overflow: 'hidden' }}>
        <div style={{ width: `${value}%`, height: '100%', background: color, borderRadius: 4 }} />
      </div>
      <div style={{ fontSize: 11, color: '#475569', marginTop: 8 }}>{desc}</div>
    </div>
  )
}
    ...
    <div style={{ padding: '24px 24px 40px', background: '#F4F6FB', minHeight: '100vh', color: '#0F172A' }}>
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
        ...
        <form onSubmit={analyze} style={{ display: 'flex', gap: 8, maxWidth: 460, margin: '0 auto' }}>
          <input
            ...
            style={{ flex: 1, padding: '9px 12px', background: '#FFFFFF', border: '1px solid #DAE0EA', borderRadius: 8, color: '#0F172A', fontSize: 13, outline: 'none' }}
          />
          <button type="submit" style={{ padding: '9px 20px', background: '#F59E0B', border: 'none', borderRadius: 8, color: '#0F172A', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
      ...
      {data && (
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 12, marginBottom: 20 }}>
            <div style={{ background: '#FFFFFF', border: '1px solid #DAE0EA', borderRadius: 8, padding: '14px', textAlign: 'center' }}>
              <div style={{ fontSize: 20, fontWeight: 700, color: '#0F172A' }}>{data.trust_citation_ratio}</div>
              <div style={{ fontSize: 11, color: '#475569', marginTop: 2 }}>Trust : Citation ratio</div>
            </div>
            <div style={{ background: '#FFFFFF', border: '1px solid #DAE0EA', borderRadius: 8, padding: '14px', textAlign: 'center' }}>
              <div style={{ fontSize: 20, fontWeight: 700, color: '#0F172A' }}>{data.referring_domains}</div>
              <div style={{ fontSize: 11, color: '#475569', marginTop: 2 }}>Referring domains</div>
            </div>
            <div style={{ background: '#FFFFFF', border: '1px solid #DAE0EA', borderRadius: 8, padding: '14px', textAlign: 'center' }}>
              <div style={{ fontSize: 20, fontWeight: 700, color: data.toxic_links > 0 ? '#EF4444' : '#22C55E' }}>{data.toxic_links}</div>
              <div style={{ fontSize: 11, color: '#475569', marginTop: 2 }}>Toxic links</div>
            </div>
            <div style={{ background: '#FFFFFF', border: '1px solid #DAE0EA', borderRadius: 8, padding: '14px', textAlign: 'center' }}>
              <div style={{ fontSize: 20, fontWeight: 700, color: '#0F172A' }}>{data.trusted_links}</div>
              <div style={{ fontSize: 11, color: '#475569', marginTop: 2 }}>Trusted links</div>
            </div>
          </div>

          {data.note && (
            <div style={{ padding: '10px 14px', background: '#F1F4F9', border: '1px solid #334155', borderRadius: 6, fontSize: 12, color: '#64748B', display: 'flex', gap: 8 }}>
      ...
      {!loading && !data && !error && (
        <div style={{ textAlign: 'center', padding: 50, color: '#8B93A7' }}>
          ...
          <p style={{ fontSize: 12, color: '#475569', marginTop: 8 }}>Refresh backlinks for this domain first if there's no data</p>
        </div>
      )}
    </div>
  )
