...
function DifficultyGauge({ score }) {
  ...
  return (
    <div style={{ textAlign: 'center', padding: 28, background: '#FFFFFF', border: '1px solid #DAE0EA', borderRadius: 12, maxWidth: 320, margin: '0 auto 20px' }}>
    ...
    <div style={{ padding: '24px 24px 40px', background: '#F4F6FB', minHeight: '100vh', color: '#0F172A' }}>
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
        ...
        <p style={{ color: '#64748B', margin: '0 0 18px', fontSize: 13 }}>
          Real competitive difficulty (0â€“100) from SERP analysis of the ranking domains
        </p>
      ...
      {loading && <div style={{ textAlign: 'center', padding: 50, color: '#64748B' }}>Analyzing SERPâ€¦</div>}
      ...
      {data?.difficulty != null && (
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          ...
          {data.note && (
            <div style={{ marginBottom: 16, padding: '10px 14px', background: '#F1F4F9', border: '1px solid #334155', borderRadius: 6, fontSize: 12, color: '#64748B', display: 'flex', gap: 8 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12, marginBottom: 20 }}>
            <div style={{ background: '#FFFFFF', border: '1px solid #DAE0EA', borderRadius: 8, padding: '12px 14px', textAlign: 'center' }}>
              <div style={{ fontSize: 20, fontWeight: 700, color: '#0F172A' }}>{data.top10_da_avg}</div>
              <div style={{ fontSize: 11, color: '#475569', marginTop: 2 }}>Avg. DA of top 10</div>
            </div>
            <div style={{ background: '#FFFFFF', border: '1px solid #DAE0EA', borderRadius: 8, padding: '12px 14px', textAlign: 'center' }}>
              <div style={{ fontSize: 20, fontWeight: 700, color: '#0F172A' }}>{Math.round((data.strong_domain_pct || 0) * 100)}%</div>
              <div style={{ fontSize: 11, color: '#475569', marginTop: 2 }}>Strong domains (DA â‰¥ 40)</div>
            </div>
            <div style={{ background: '#FFFFFF', border: '1px solid #DAE0EA', borderRadius: 8, padding: '12px 14px', textAlign: 'center' }}>
              <div style={{ fontSize: 20, fontWeight: 700, color: '#0F172A' }}>{data.serp_overview?.length || 0}</div>
          ...
          {data.serp_overview?.length > 0 && (
            <div style={{ background: '#FFFFFF', border: '1px solid #DAE0EA', borderRadius: 8, overflow: 'hidden' }}>
              <div style={{ padding: '12px 16px', borderBottom: '1px solid #DAE0EA', fontWeight: 600, fontSize: 13, color: '#0F172A' }}>SERP Overview</div>
              {data.serp_overview.map((r, i) => (
                <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', padding: '10px 16px', borderBottom: '1px solid #DAE0EA' }}>
                  <div style={{ width: 26, height: 26, borderRadius: 4, background: i < 3 ? '#6366F115' : '#E5E9F2', color: i < 3 ? '#6366F1' : '#475569', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 12, flexShrink: 0 }}>{i + 1}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#0F172A', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.title || r.domain}</div>
                    ...
                    {r.snippet && <div style={{ fontSize: 11, color: '#475569', marginTop: 3, lineHeight: 1.4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.snippet}</div>}
                  </div>
