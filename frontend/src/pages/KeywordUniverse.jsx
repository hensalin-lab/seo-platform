    ...
    <div style={{ padding: '24px 24px 40px', background: '#F4F6FB', minHeight: '100vh', color: '#0F172A' }}>
      ...
      {data && (
        <div style={{ maxWidth: 760, margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 20, flexWrap: 'wrap' }}>
            <div style={{ background: '#FFFFFF', border: '1px solid #DAE0EA', borderRadius: 8, padding: '12px 18px', textAlign: 'center' }}>
              <div style={{ fontSize: 22, fontWeight: 700, color: '#8B5CF6' }}>{data.keyword_count}</div>
              <div style={{ fontSize: 11, color: '#475569', marginTop: 2 }}>Keywords discovered</div>
            </div>
            {data.found_pages?.length > 0 && (
              <div style={{ background: '#FFFFFF', border: '1px solid #DAE0EA', borderRadius: 8, padding: '12px 18px', fontSize: 11, color: '#475569', flex: 1 }}>
                <div style={{ fontWeight: 600, color: '#64748B', marginBottom: 4 }}>Probed competitor pages</div>
          ...
          {data.keywords?.length > 0 ? (
            <div style={{ background: '#FFFFFF', border: '1px solid #DAE0EA', borderRadius: 8, overflow: 'hidden' }}>
              <div style={{ padding: '12px 16px', borderBottom: '1px solid #DAE0EA', fontWeight: 600, fontSize: 13, color: '#0F172A', display: 'flex', gap: 6, alignItems: 'center' }}>
                <Target size={14} style={{ color: '#8B5CF6' }} /> Keywords {data.domain} ranks for
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))' }}>
                {data.keywords.map((kw, i) => (
                  <div key={i} style={{ padding: '10px 16px', borderBottom: '1px solid #DAE0EA', borderRight: '1px solid #DAE0EA', fontSize: 12, color: '#0F172A', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      {!loading && !data && !error && (
        <div style={{ textAlign: 'center', padding: 50, color: '#8B93A7' }}>
          ...
          <p style={{ fontSize: 12, color: '#475569', marginTop: 8 }}>Fills the "full keyword universe" gap vs SE Ranking / Serpstat</p>
        </div>
      )}
    </div>
  )
