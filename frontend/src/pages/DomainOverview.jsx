...
function StatCard({ icon: Icon, label, value, color, cta, ctaLabel, ctaHref }) {
  return (
    <div style={{
      background: '#FFFFFF', border: '1px solid #DAE0EA', borderRadius: 10,
...
function ScoreRing({ score, size = 64 }) {
  ...
  return (
    <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
      <circle cx={size/2} cy={size/2} r={r} fill="none" stroke="#E5E9F2" strokeWidth="5" />
      <circle cx={size/2} cy={size/2} r={r} fill="none" stroke={SCORE_COLOR(score)}
        ...
        strokeLinecap="round" />
      <text x={size/2} y={size/2 + 5} textAnchor="middle" fill="#0F172A"
...
export default function DomainOverview() {
  ...
  return (
    <div style={{ padding: '24px 24px 40px', background: '#F4F6FB', minHeight: '100vh', color: '#0F172A' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', gap: 8, maxWidth: 560, margin: '0 auto' }}>
          <div style={{ flex: 1, position: 'relative' }}>
            ...
            <input
              ...
              autoFocus
              style={{
                width: '100%', padding: '12px 14px 12px 40px', background: '#FFFFFF',
                border: '1px solid #DAE0EA', borderRadius: 10, color: '#0F172A', fontSize: 15,
      ...
      {/* Empty state */}
      {!loading && !data && !error && (
        <div style={{ textAlign: 'center', padding: 60, color: '#8B93A7' }}>
      ...
      {data && (
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          ...
          {data.top_pages && data.top_pages.length > 0 && (
            <div style={{
              background: '#FFFFFF', border: '1px solid #DAE0EA', borderRadius: 10, padding: 18,
            }}>
              <h3 style={{ margin: '0 0 14px', fontSize: 15, fontWeight: 600, color: '#0F172A' }}>
                Top Pages
              </h3>
                ...
                <thead>
                  <tr style={{ borderBottom: '1px solid #DAE0EA' }}>
                    {['URL', 'Title', 'Words', 'Status'].map(h => (
                      <th key={h} style={{ padding: '8px 10px', textAlign: 'left', color: '#475569', fontSize: 11, fontWeight: 600, textTransform: 'uppercase' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
