...
export default function AiActionModal({ issue, onClose, onVerify }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(2px)' }} onClick={onClose}>
      <div style={{ background: '#fff', borderRadius: 14, maxWidth: 640, width: '92%', maxHeight: '90vh', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }} onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div style={{ padding: '18px 22px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              ...
              {url && <span style={{ fontSize: 11, color: '#64748b', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 200 }}>{url}</span>}
            </div>
        ...
        <div style={{ padding: '14px 22px', borderBottom: '1px solid #f1f3f5' }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>Issue Summary</div>
        ...
        <div style={{ padding: '14px 22px', borderBottom: '1px solid #f1f3f5' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em' }}>AI-Generated Fix</div>
            <button onClick={handleCopy} style={{ padding: '5px 12px', borderRadius: 6, border: '1px solid #e2e8f0', background: copied ? '#d1fae5' : '#fff', cursor: 'pointer', fontSize: 11, fontWeight: 600, color: copied ? '#065f46' : '#475569', display: 'flex', alignItems: 'center', gap: 5, transition: 'all 0.15s' }}>
              {copied ? <Check size={12} /> : <Copy size={12} />}
              {copied ? 'Copied!' : 'Copy Code'}
            </button>
          </div>
          <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, padding: 14, maxHeight: 240, overflowY: 'auto' }}>
            <pre style={{ margin: 0, fontSize: 12, color: '#0f172a', lineHeight: 1.6, whiteSpace: 'pre-wrap', fontFamily: "'JetBrains Mono', 'Fira Code', monospace" }}>{fixCode}</pre>
          </div>
        </div>
        ...
        <div style={{ padding: '14px 22px', display: 'flex', gap: 10, justifyContent: 'flex-end', background: '#fafbfc' }}>
          ...
          <button onClick={handleCopy} style={{ padding: '8px 16px', borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', fontSize: 12, fontWeight: 600, color: '#475569', display: 'flex', alignItems: 'center', gap: 6 }}>
