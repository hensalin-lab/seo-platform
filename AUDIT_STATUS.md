...
### ✅ Resolved (2026-09-07): backend origin down + DB outage
The outage had two distinct root causes, both now fixed:

1. **Neon Postgres data-transfer quota exhausted** → `asyncpg.InsufficientResourcesError: Your project has exceeded the data transfer quota`. Migrated to a free Supabase Postgres (user budget is free-tier only). Fixed URL-encoding for the DB password (`@`→`%40`, `#`→`%23`, `*`→`%2A`), wired `DATABASE_URL` into `backend/.env` + dashboard env var.
2. **App hard-crashed at startup when DB was down** → `init_db()` now retries 3× (5s apart) then logs and continues startup instead of aborting (commit `816b7aa`). Also: `app/database.py` `_run_pending_migrations` exports `DATABASE_URL` env instead of `set_main_option` (configparser crashed on `%40`); `alembic/env.py` reads an env-var override URL. Init now creates all 58 tables on Supabase.
3. **Supabase direct host is IPv6-only (`db.<ref>.supabase.co` → AAAA only)** → the FastAPI Cloud container has no IPv6 route, so *every* outbound call failed with `[Errno 101] Network is unreachable` (DB, workers, everything). Switched the connection string to the **IPv4 pooler endpoint** `aws-0-ap-southeast-1.pooler.supabase.com` (session pooler, port 5432, user `postgres.<project-ref>`), which the container can reach.

**Current live status**: `/api/health` → `{"status":"healthy","version":"2.0.0","database":"connected"}`. MCP `/api/mcp` tools re-verified live: **all tools operational** — `serp_position`→serper real rank (position 2, semrush.com), `backlink_summary`→pagerank DA 88.9, `free_ssl`→SSL Labs A+ (github.com), free_* keyless tools all OK, `providers_status` OK. `keyword_volume` uses keyless fallback (no paid volume provider — expected); `free_ssl` on `example.com` returns "scan pending" (SSL Labs cache quirk, other domains fine). Last blocker was an MCP-layer bug (commit `d6374e9`): MCP data tools passed only the per-user config (empty for anonymous callers) instead of `effective_config`, so `serper`/`openserp` sent an empty api_key → 403 even though the env key was valid. Fixed by merging env defaults in `_provider_cfg`.

**Deploys**: `4d276c1d`/`c82c98b5`/`934f30e4` were code-only fixes. Latest successful deploy from `backend/` after the pooler env change is live (uses `fastapi-cloud-cli` via `python -m fastapi cloud deploy` with `FASTAPI_CLOUD_TOKEN`, because the bare `fastapicloud` command and `fastapi` shim are not on PATH). Final fixes committed: `816b7aa` (startup resilience + alembic env URL), `d6374e9` (MCP effective_config for anonymous callers + outage docs).
