...
**Current live status**: `/api/health` → `{"status":"healthy","version":"2.0.0","database":"connected"}`. MCP `/api/mcp` tools re-verified live: 9/11 return real data (backlink_summary=pagerank, free_* keyless all OK, ai_citations, autocomplete, whois, dns). Remaining: `serp_position`→serper 403 (dashboard `SERPER_API_KEY` differs from known-good key `9bcea086…b360`, which returns 200 locally — needs verification in dashboard); `free_ssl` returns ssllabs "scan pending" (transient, keyless). `keyword_volume` falls back to keyless (no paid volume provider configured — expected).

**Deploys**: `4d276c1d`/`c82c98b5`/`934f30e4` were code-only fixes. Latest successful deploy from `backend/` after the pooler env change is live (uses `fastapi-cloud-cli` via `python -m fastapi cloud deploy` with `FASTAPI_CLOUD_TOKEN`, because the bare `fastapicloud` command and `fastapi` shim are not on PATH).
